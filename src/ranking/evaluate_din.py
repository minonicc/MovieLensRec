...
def calculate_genre_punch_eval(target_item_genres, snap_items, item_idx2genres, snap_ratings):
    ...
    for it, rt in zip(h_it, h_rt):
        for g in item_idx2genres.get(it, []): g_counts[g] += 1; g_scores[g] += rt
...
def evaluate_din_batch(batch_df, model_path, feature_dims, meta, hit_k_list, ndcg_k):
    """
    子进程评估单元：执行 100 维长序列的打分逻辑。
    """
    device = torch.device("cpu")
    model = DIN_DCN_PPNet(feature_dims, embed_dim=16, cross_layers=2)
    ...
    for _, row in batch_df.iterrows():
        ...
        snap_items, snap_ratings = u_seq[:pos], u_rts[:pos]
        
        # 严格截取 100 长度
        hist_ids = (snap_items[-DIN_HIST_LEN:] + [0]*DIN_HIST_LEN)[:DIN_HIST_LEN]
        hist_gs = []
        for h in hist_ids:
        ...
        # 2. 构造候选打分矩阵
        c_cat, c_seq, c_num = defaultdict(list), {'hist_movie_ids': [], 'hist_genre_ids': []}, defaultdict(list)
...
def run_din_evaluation(model_path='best_din_model.pth'):
    print(f"--- DIN 综合架构深度评估启动 (100维序列) ---")
    ...
    print(f"启动 {num_workers} 个 Worker 进行长序列并行评估...")
    batches = np.array_split(test_df, num_workers)
    all_g, all_n, all_m = [], [], []
    all_h = {k: [] for k in hit_k_list}

    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = [executor.submit(evaluate_din_batch, b, model_path, feature_dims, meta, hit_k_list, ndcg_k) for b in batches]
    ...
    print("" + "="*60)
    print(f"【DIN+DCN+PPNet】综合架构评估报告")
    print("="*60)
...
if __name__ == "__main__":
    import sys
    model_file = sys.argv[1] if len(sys.argv) > 1 else "best_din_model.pth"
    run_din_evaluation(model_file)
